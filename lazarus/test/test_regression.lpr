...
begin
  ...
  case AKind of
    ...
    vkFrequencyDrift:
      { PSK31 は AFC を持つ (MDM-006) が、位相差判別器の捕捉範囲が
        sc_bw/4 (記号速度の 1/4 = 7.8 Hz) で頭打ちになるため、この
        0->60 Hz という条件には追従しきれない ―― これは実装の不備では
        なく方式そのものの限界で、AFC の有無で CER は変わらない
        (実測 0.79 のまま)。**PSK63 は捕捉範囲が 15.6 Hz と広いぶん、
        同じ条件でも AFC で本文CER が 0.79 前後→0.10 前後まで改善する**
        (test_afc がこの改善を別に確かめている)。それでも 0.05 の
        一般しきい値には届かないので、ここでは両方とも既知の限界として
        扱う。MFSK はまだ AFC を持たない。トーン間隔が 15.625 Hz しか
        ないので、60 Hz はトーン 4 本ぶんに当たる ―― 同期は追えても
        周波数は追えない。
