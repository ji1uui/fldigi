...
begin
  ...
  try
    { 既定が「毎回聞く」であること。ここが granted だと
      ...
      ここはもう一つの意味を背負っている。**L6 の中身はまだ平文である**
      (ARC-005 を Phase 4 へ保留した。README §30)。保留してよいと
      判断した根拠の一つが「既定では L6 に何も書かれない」ことなので、
      この既定を granted に変えるなら、先に暗号化を入れること。
      この表明は、それを黙って変えられないようにするためにある。 }
