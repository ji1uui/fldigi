...
type
  ...
  TModemCapability = (
    ...
    mcBandwidth,  // 帯域幅可変
    { Squelch プロパティを実際に解釈する。

      基底クラスは Squelch を全モデムに公開しているが、解釈するかどうかは
      モデム次第である。この印が無いモデムに Squelch を設定しても何も
      起きない。**設定が効くのか効かないのかを呼び出し側が知る手段が無い**
      という食い違いを埋めるために足した (2026-09 の品質レビュー)。

      いま印を立てているのは PSK だけ。CW は門番をトーン検出側に移した
      ので持たず (README §28)、RTTY は fldigi 由来の実装に無い。 }
    mcSquelch,    // Squelch プロパティを解釈する
    mcRx,         // 受信対応
    mcTx          // 送信対応
  );
