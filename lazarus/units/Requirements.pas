...
begin
  ...
  { MFSK の「音の層」のうち、トーンを測って軟判定に落とすところ。
    **シンボル同期の追尾と AFC は含まない** ―― 区切りが合っている前提で
    トーンをどれだけ正しく測れるかだけを見る。追尾は MDM-013 が、
    AFC は Phase 3 が見る。文面をここまでに絞ってあるのは、追尾まで
    含んだ書き方にすると試験していない部分を覆ってしまうからである
    (§36 §37 の轍)。試験側でも追尾を明示的に切ってある。 }
  ...
  { MFSK の「音の層」のもう半分。区切りをどこに置くかを追う。

    ここを独立した要求にしてあるのは、**硬判定がずれに強い**からである。
    窓が二つのシンボルにまたがっても長く重なったほうが勝つので、
    ずれが半シンボルに近づくまで答えは変わらない。「ずらしても復調
    できた」という試験は追尾を消しても通ってしまう。そこで
      - ずれを**半シンボルちょうど**に置く (硬判定が割れる唯一の点)
      - 硬判定ではなく**軟判定の余裕**を測る (次段の Viterbi が食う量)
    の二つで見ている。

    主張を「一つも落とさない」ではなく「掴んだあとは落とさない」に
    してあるのは、最悪の点から引き込む間は判定材料が無いからである。
    上流が送信の頭に前置きを流すのも同じ理由による。 }
  R('MDM-013', 'MFSKがシンボルの区切りを追尾する',
    expCommunicate, objRobustness, fndIntelligentReceiver,
    [fndModernComputing], False, priMust, 2,
    'test_mfsk_sync (半シンボルずれの引き込み・ずれの掃引・送受の時計差' +
    '±3000ppm・残るずれの式・三つの門・区切り間隔の上下限)', rsVerified,
    '§12 Phase 2 MFSK', '');

  { MFSK をモードとして成立させる要求。部品 (MDM-009 / MDM-010 /
    MDM-012 / MDM-013) が揃ったことを「MFSK ができた」と書かないための
    枠である。 }
