...
type
  ...
  TDecodeEvidence = record
    ...
    { この結果が **確定した区画の先頭** の通算サンプル位置
      (Open あるいは Replay の起点から数えたサンプル数)。不明なら -1。

      なぜ「区画の先頭」なのか
      ------------------------------------------------------------------
      復調器は区画 (ModemEngine.MODEM_BLOCK_SIZE) 単位で音を受け取る。
      1 つの文字はふつう複数の区画にまたがる音から確定するので、
      「その文字を生んだ 1 点」というものは無い。ここが指すのは
      **確定した瞬間に処理していた区画の先頭**である。

      末尾ではなく先頭にしてあるのは、末尾がその文字を生んだ音より
      後ろを指すからである。末尾から流し直しても、その文字は二度と
      出てこない ―― Replay Decode (X-06) にも障害再現にも使えない。

      分解能は区画長 (8 kHz で 512 サンプル = 64 ms)。
      Phase 3 の Algorithm Portfolio は複数の戦略に**同じ区画**を配るので、
      戦略どうしの結果はこの値でぴったり並ぶ。区画長が違う戦略を混ぜると
      並ばなくなる ―― そのときは配る側が揃える責任を持つ。 }
    SamplePos: Int64;
