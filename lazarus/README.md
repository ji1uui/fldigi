...
## 27. CW 整定過渡の根本原因 — 調査結果と、直さなかった判断
...
残る不具合は「雑音が完全にゼロ」という、実機の受信では起こりえない入力で
しか出ない。一方、試した修正はどれも現実の入力を悪化させた。
**実在しない条件のために実在する性能を落とすのは割に合わない。**

根本から直すには、検出器の前段そのもの ── 遅れの無い雑音床推定
(最小値追跡など) と、推定量の有効性の概念 ── を作り直す必要がある。
それは復調特性を全面的に変える変更なので、手作りの十数例ではなく
**Golden WAV による BER/CER の回帰試験 (MDM-001 / Z-02)** で
裏付けられる状態になってから行うべきである。それは Phase 2 の仕事。

要求 `MDM-002` として登録し、Phase 2 / 後送り / 検証方法は
Golden WAV BER/CER、と記録した。忘れないための仕掛けは §22 の
トレーサビリティが担う。
